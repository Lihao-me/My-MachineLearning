...
我需要完成plotData.m文件中绘图函数的编写，将已有数据进行绘制。
...
我需要在sigmoid.m文件中完成sigmoid函数，使调用它可以计算向量中每一个元素的函数值。
...
我需要在costFunction.m文件中完成损耗和梯度下降的计算，其中，损耗函数为：
...
在该步骤我不需要补充任何代码，保证上一步骤的函数准确，运行即可。主函数也会调用plotDecisionBoundary.m文件进行决策边界的绘制。
...
同第一部分一样，为了将数据内容可视化处理，我们需要在[plotData.m](https://github.com/Lihao-me/My-MachineLearning/blob/main/01_Coursera-ML-AndrewNg-2011/02_exercises/01_myEx1/ex1/plotData.m)文件中对数据进行绘制。
...
我需要在costFunctionReg.m文件中完成损失的计算和导数值。
...
在plotDecisionBoundary.m中进行决策边界的绘制，其中$\lambda=1$。
...
此处的预测值和题目的期望值显然不符，经过debug后，我发现predict.m函数写错了，忘记写g函数了。而在上一部分的运行中却没有什么问题，我发现这是因为上一部分X*theta的值经过sigmoid与否并不影响判决，而当多变量的情况就会产生影响了。
